...
resource "github_repository_ruleset" "this" {
  ...
  rules {
    ...
    deletion = try(each.value.rules.deletion, local.default_rules.deletion)
    non_fast_forward        = try(each.value.rules.non_fast_forward, local.default_rules.non_fast_forward)
    ...
    dynamic "pull_request" {
      for_each = lookup(each.value.rules, "pull_request", local.default_rules.pull_request) != {} ? [1] : []
      content {
        dismiss_stale_reviews_on_push = try(pull_request.value.dismiss_stale_reviews_on_push, local.default_rules.pull_request.dismiss_stale_reviews_on_push)
        require_code_owner_review     = try(pull_request.value.require_code_owner_review, local.default_rules.pull_request.require_code_owner_review)
        required_approving_review_count = try(pull_request.value.required_approving_review_count, local.default_rules.pull_request.required_approving_review_count)
